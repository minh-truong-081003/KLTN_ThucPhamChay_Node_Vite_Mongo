...
const Header = ({ hideLogo = false, bgColor = 'bg-white' }) => {
  ...
  return (
    <div
      className={`header flex items-center justify-between gap-2 px-4 py-2 select-none sticky top-0 w-full ${bgColor} z-10`}
    >
      {/* Logo */}
      {!hideLogo && (
              ...
              {isSearching ? (
                ...
                <>
                  <div className='p-3 border-b border-gray-200 bg-gray-50'>
                    <p className='text-sm font-semibold text-gray-700'>Tìm thấy {searchResults.length} sản phẩm</p>
                  </div>
                  <div className='py-2'>
        ...
        <div className='info_notifi flex items-center gap-x-5'>
          ...
          <Tooltip title='Danh sách yêu thích' arrow={false}>
            <Link to='/account-layout/my-favorites'>
              <AiFillHeart
                className={`text-xl ${bgColor === 'transparent' ? 'text-white' : 'text-red-500'} hover:text-red-500 transition-colors`}
              />
            </Link>
          </Tooltip>
