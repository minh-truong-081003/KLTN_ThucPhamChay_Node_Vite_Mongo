  ...
  return (
    <div className='sm:w-full lg:w-full max-w-[300px]'>
      <div className='sticky top-[80px] self-start'>
        <div className={`${styles.category_menu_title} sm:text-[25px] text-center lg:text-[28px]`}>
          Danh mục tin tức
        </div>
        <div className='w-full max-w-[260px] mx-auto mb-[70px]'>
          <ul>
            <li
              className={`${styles.menu_category} transition-all duration-200 ${isAllBlogs ? 'border-l-4 border-[#d3b673] bg-[#f5f5dc] pl-3' : ''}`}
            >
              <Link to='/blogs' className={`block ${isAllBlogs ? 'text-[#8a733f] font-semibold' : ''}`}>
                Tất cả tin tức
              </Link>
            </li>
            {blogCategories &&
              ...
              blogCategories?.docs?.map((item: any, index: number) => {
                ...
                return (
                  <li
                    key={index}
                    className={`${styles.menu_category} transition-all duration-200 ${isActive ? 'border-l-4 border-[#d3b673] bg-[#f5f5dc] pl-3' : ''}`}
                  >
                    <Link
                      to={`category/${item?._id}`}
                      className={`block ${isActive ? 'text-[#8a733f] font-semibold' : ''}`}
                    >
                      {item?.name}
                    </Link>
                  </li>
