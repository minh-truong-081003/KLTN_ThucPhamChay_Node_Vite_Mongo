...
export const ReviewListActive = () => {
  ...
  const getColumnSearchProps = (dataIndex: string, placeholder: string) => ({
    ...
    filterIcon: (filtered: boolean) => (
      <AiOutlineSearch style={{ color: filtered ? '#1890ff' : undefined, fontSize: '16px' }} />
    ),
      ...
      render: (_, record) => (
        <Space size='small' style={{ display: 'flex', justifyContent: 'center' }}>
          ...
          <Popconfirm
            ...
            onConfirm={() => handleToggleVisibility(record._id)}
            okText='Có'
            cancelText='Không'
          >
            <button
              style={{ backgroundColor: '#3b82f6', color: 'white', border: 'none' }}
              className='px-3 py-1.5 rounded hover:bg-blue-600 flex items-center gap-1.5 transition-colors'
            >
    ...
    <div>
      {hasSelected && (
        <div style={{ marginBottom: 16 }} className='flex items-center gap-3'>
          <Tooltip title={`Đang chọn ${selectedRowKeys.length} đánh giá`}>
            <AntButton
              size='large'
              style={{ backgroundColor: '#3b82f6', color: 'white', borderColor: '#3b82f6' }}
              className='hover:!bg-blue-600 text-sm font-semibold'
